...
import { ReportData, ComparisonResult } from '../types';
...
export class HtmlReporter {
  ...
  constructor(templatePath?: string, outputPath?: string) {
    this.templatePath = templatePath || join(__dirname, '..', 'templates');
    this.outputPath = outputPath || join(process.cwd(), 'reports');
  }
  ...
  async generateReport(reportData: ReportData): Promise<string> {
    ...
    const reportPath = join(this.outputPath, 'qa-report.html');
  ...
  private async generateHtmlContent(reportData: ReportData): Promise<string> {
  ...
  private async getTemplate(): Promise<string> {
    ...
    try {
      const customTemplatePath = join(this.templatePath, 'report.hbs');
...
<body>
  ...
  private async copyAssets(): Promise<void> {
    ...
    const assetsDir = join(this.outputPath, 'assets');
    await fs.ensureDir(assetsDir);
    
    // You can add CSS, JS, or other assets here
  }

  private async generatePageReports(results: ComparisonResult[]): Promise<void> {
    const pagesDir = join(this.outputPath, 'pages');
